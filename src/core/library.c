    ...
    )
{
    ...
    if (InterlockedIncrement16(&Library->LoadRefCount) == 1) {
        //
        // Load the library.
        //
        CxPlatSystemLoad();
        ...
        Library->Version[0] = VER_MAJOR;
        Library->Version[1] = VER_MINOR;
        Library->Version[2] = VER_PATCH;
        Library->Version[3] = VER_BUILD_ID;
    }
}
    ...
    )
{
    ...
    if (InterlockedDecrement16(&Library->LoadRefCount) == 0) {
        QUIC_LIB_VERIFY(Library, Library->OpenRefCount == 0);
    ...
    )
{
    ...
    Library->PartitionCount = (uint16_t)CXPLAT_MIN(Library->ProcessorCount, DefaultMaxPartitionCount);
    ...
    for (uint16_t i = 0; i < Library->ProcessorCount; ++i) {
        ...
        CxPlatLockInitialize(&Library->PerProc[i].ResetTokenLock);
        Library->PerProc[i].ResetTokenHash = NULL;
        CxPlatZeroMemory(
    ...
    for (uint16_t i = 0; i < Library->ProcessorCount; ++i) {
        Status =
            CxPlatHashCreate(
                ...
                ResetHashKey,
                sizeof(ResetHashKey),
                &Library->PerProc[i].ResetTokenHash);
        if (QUIC_FAILED(Status)) {
            QuicTraceEvent(
                LibraryErrorStatus,
                "[ lib] ERROR, %u, %s.",
    ...
    if (QUIC_FAILED(Status)) {
        if (Library->PerProc != NULL) {
            for (uint16_t i = 0; i < Library->ProcessorCount; ++i) {
                ...
                CxPlatLockUninitialize(&Library->PerProc[i].ResetTokenLock);
                CxPlatHashFree(Library->PerProc[i].ResetTokenHash);
            }
        ...
        if (Library->DefaultCompatibilityList != NULL) {
            ...
            Library->DefaultCompatibilityList = NULL;
        }
    ...
    )
{
    ...
    //
    if (++Library->OpenRefCount == 1) {
        Status = MsQuicLibraryInitialize(Library);
        if (QUIC_FAILED(Status)) {
            Library->OpenRefCount--;
            goto Error;
        }
    }
    ...
    )
{
    ...
    CXPLAT_FRE_ASSERT(Library->OpenRefCount > 0);
    QuicTraceEvent(
        LibraryRelease,
        "[ lib] Release");
    ...
    )
{
    ...
    case QUIC_PARAM_GLOBAL_VERSION:
        ...
        if (*BufferLength < sizeof(Library->Version)) {
            *BufferLength = sizeof(Library->Version);
            Status = QUIC_STATUS_BUFFER_TOO_SMALL;
            break;
        }
        ...
        *BufferLength = sizeof(Library->Version);
        CxPlatCopyMemory(Buffer, Library->Version, sizeof(Library->Version));
    ...
    )
{
    ...
    CxPlatLockAcquire(&Library->PerProc[CurProcIndex].ResetTokenLock);
    QUIC_STATUS Status =
        CxPlatHashCompute(
            Library->PerProc[CurProcIndex].ResetTokenHash,
            CID,
            Library->CidTotalLength,
            sizeof(HashOutput),
            HashOutput);
    CxPlatLockRelease(&Library->PerProc[CurProcIndex].ResetTokenLock);
